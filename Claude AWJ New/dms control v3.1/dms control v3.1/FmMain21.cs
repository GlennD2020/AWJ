...
namespace dms_control_v3
{
    ...
    public partial class FmMain : Form
    {
        ...
        private void btnTdmSaveStandalone_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // ══════════════════════════════════════════════════════
                {
                    ...
                    uint points = (uint)Math.Max((uint)(firstTones * 16 * firstBwMhz / 20.0), 32);
                    device.dds.Start = 0;
                    device.dds.Step = (float)(bwHz / points);
                ...
                // ══════════════════════════════════════════════════════
                //  5. STOP TDM, then write TDM band table
                // ══════════════════════════════════════════════════════
                ...
                foreach (DataGridViewRow row in dgvBands.Rows)
                {
                    ...
                    // Use setRampBandwidth to compute Points/Step — same as
                    // the working panel path and DDSSetConfiguration.
                    // This gives correct register values for the AD9106.
                    // Note: saturates at Points=2892 for 60+ MHz BW.
                    ...
                    bandData[25] = 0; bandData[26] = 0; bandData[27] = 0;
                ...
                // Wait for all EEPROM writes to complete
                WaitForCommandQueue(5000);

                // Clean completion message
                string modeStr = pulseMode
                    ? string.Format("Pulsed ({0}ms on / {1}ms off)", pulseOn, pulseOff)
                    : "Continuous";
                lblGuardStatus.Text = string.Format("SAVED: {0} bands, {1}ms dwell, {2}",
                ...
                MessageBox.Show(string.Format(
                    ...
                    "  • Dwell: {1}ms\n" +
                    "  • Mode: {2}\n\n" +
                    "Power cycle the device to start autonomous operation.",
                    bandCount, dwellMs, modeStr),
                    "Save Complete ✓",
        ...
        private void SetDDSParamsForBand(BandConfiguration band)
        {
            ...
            // Proportional Points from known working baseline:
            // Tones=8 @ 20MHz → Points=128 → register 2048 → correct BW
            // Scale by BW ratio: wider bands get proportionally larger ramp endpoints.
            int tones = band.Tones > 0 ? band.Tones : 8;
            uint points = (uint)Math.Max((uint)(tones * 16 * band.BandwidthMHz / 20.0), 32);
        ...
        private void ConfigureDDSForJamming(BandConfiguration firstBand)
        {
            try
            {
                ...
                // Compute DDS parameters using setRampBandwidth (with Start=0)
                // which correctly computes AD9106 Points for the requested BW.
