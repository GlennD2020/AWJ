    ...
    public partial class FmMain : Form
    {
        ...
        private void EnsureGridColumns()
        {
            ...
            if (!dgvBands.Columns.Contains("colActive"))
            {
                ...
                col.DefaultCellStyle.NullValue = true;  // New rows default to ACTIVE
                dgvBands.Columns.Add(col);
            }
                ...
                if (usePulseMode)
                {
                    ...
                    for (int cycle = 0; cycle < pulseCycles && isJamming; cycle++)
                    {
                        ...
                        for (int i = 0; i < allBands.Count && isJamming; i++)
                        {
                            ...
                            // Dwell on this band for the user-specified time
                            if (userDwellMs > 0)
                                Thread.Sleep(userDwellMs);
                        }
                ...
                else
                {
                    ...
                    while (isJamming)
                    {
                        ...
                        for (int i = 0; i < allBands.Count && isJamming; i++)
                        {
                            ...
                            // Dwell on this band for the user-specified time
                            // WaitForCommandQueue in HopToFrequency handles USB round-trip,
                            // this additional sleep ensures minimum dwell time on each band
                            if (userDwellMs > 0)
                                Thread.Sleep(userDwellMs);
                        }
