...
namespace dms_control_v3
{
    public partial class FmMain : Form
    {
        ...
        public FmMain(GlobalConfigurations _cfg)
        {
            ...
            {
                cmbbxDdsMode.Items.Clear();  // Force clear any designer-set items
            ...
            if (dgvBands != null)
            {
                ...
                // ADVANCED: Add Tones column if it doesn't exist
                if (!dgvBands.Columns.Contains("colTones"))
                {
                    DataGridViewTextBoxColumn tonesCol = new DataGridViewTextBoxColumn();
                    tonesCol.Name = "colTones";
                    tonesCol.HeaderText = "Tones";
                    tonesCol.Width = 60;
                    tonesCol.DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
                    dgvBands.Columns.Add(tonesCol);
                }

                // ADVANCED: Add default multi-band configuration with intelligent tone counts
                dgvBands.Rows.Clear();

                // WiFi 2.4GHz - 13 channels, use 13 tones (one per channel)
                dgvBands.Rows.Add("", "Triggered", "WiFi 2.4GHz", 2400.0, 100.0, 100.0, false, 13);

                // DJI 5.8GHz - Fast hopping, use 20 tones (5 MHz spacing)
                dgvBands.Rows.Add("", "Triggered", "DJI 5.8GHz", 5800.0, 100.0, 100.0, false, 20);

                // LTE Band 3 - OFDMA needs dense coverage, use 32 tones
                dgvBands.Rows.Add("", "Triggered", "LTE Band 3", 1800.0, 100.0, 100.0, false, 32);

                // Walkie 900MHz - Simple comms, use 8 tones (6.25 MHz spacing)
            ...
            Debug.WriteLine("╔═══════════════════════════════════════════════════════╗");
            Debug.WriteLine("║  DMS CONTROL v3.2 - ADVANCED PATTERN MEMORY ENABLED  ║");
            Debug.WriteLine("╚═══════════════════════════════════════════════════════╝");
        }
        ...
        /// <summary>
        /// ADVANCED: Calculate optimal number of tones based on grid settings or intelligent defaults
        /// </summary>
        private int CalculateOptimalTones(DataGridViewRow row, string bandName, double bandwidthMHz)
        {
            int numTones = 8; // Default fallback

            // Priority 1: Check if grid has explicit Tones column value
            if (dgvBands.Columns.Contains("colTones") &&
                row.Cells["colTones"].Value != null &&
                !string.IsNullOrWhiteSpace(row.Cells["colTones"].Value.ToString()))
            {
                try
                {
                    numTones = Convert.ToInt32(row.Cells["colTones"].Value);
                    if (numTones > 0)
                    {
                        Debug.WriteLine("│   Using user-specified tones: " + numTones);
                        // Validate range
            ...
            // Priority 2: Intelligent defaults based on band name
            string bandLower = bandName.ToLower();

            if (bandLower.Contains("wifi"))
            {
                // WiFi: 13 channels in 2.4 GHz, 25 channels in 5 GHz
                numTones = bandwidthMHz >= 200 ? 25 : 13;
                Debug.WriteLine("│   Auto-detected WiFi: " + numTones + " tones (channel-matched)");
            }
            else if (bandLower.Contains("dji") || bandLower.Contains("drone") || bandLower.Contains("mavic") || bandLower.Contains("phantom"))
            {
                // DJI/Drones: Fast frequency hopping, needs dense coverage
                numTones = 20;
                Debug.WriteLine("│   Auto-detected Drone: " + numTones + " tones (fast-hopping)");
            }
            else if (bandLower.Contains("lte") || bandLower.Contains("cellular") || bandLower.Contains("4g") || bandLower.Contains("5g"))
            {
                // LTE: OFDMA with many subcarriers, needs maximum density
                numTones = 32;
                Debug.WriteLine("│   Auto-detected LTE: " + numTones + " tones (OFDMA optimized)");
            }
            else if (bandLower.Contains("walkie") || bandLower.Contains("radio") || bandLower.Contains("pmr") || bandLower.Contains("frs"))
            {
                // Simple radios: Fewer channels, less density needed
                numTones = 8;
                Debug.WriteLine("│   Auto-detected Radio: " + numTones + " tones (channel-spaced)");
            }
            else if (bandLower.Contains("bluetooth") || bandLower.Contains("bt"))
            {
                // Bluetooth: 79 channels, 1 MHz each
                numTones = (int)(bandwidthMHz);
                if (numTones > 40) numTones = 40; // Cap at reasonable limit
                Debug.WriteLine("│   Auto-detected Bluetooth: " + numTones + " tones (1 MHz spacing)");
            }
            else if (bandLower.Contains("gps") || bandLower.Contains("gnss"))
            {
                // GPS/GNSS: Narrow bands, fewer tones
                numTones = 4;
                Debug.WriteLine("│   Auto-detected GPS: " + numTones + " tones (narrow-band)");
            }
            else
            {
                // Priority 3: Calculate based on bandwidth (1 tone per 5-10 MHz)
                numTones = (int)(bandwidthMHz / 6.25); // ~6.25 MHz per tone
                if (numTones < 4) numTones = 4;       // Minimum coverage
                if (numTones > 32) numTones = 32;     // Maximum density
                Debug.WriteLine("│   Auto-calculated: " + numTones + " tones (6.25 MHz spacing)");
            }

            // Validate range
            if (numTones < 2) numTones = 2;
            if (numTones > 64) numTones = 64; // Hard limit for AD9106
        ...
        /// <summary>
        /// ADVANCED: Execute Chirp Pattern Mode with smart sweep targeting
        /// </summary>
        private void ExecuteChirpMode(DataGridViewRow row, string bandName, double startFreqMHz, double bandwidthMHz, int dacChannel)
        {
            ...
            // ADVANCED: Determine chirp type based on band characteristics
            double startRatio = 0.0;
            double endRatio = 1.0;
            string chirpType = "Full";

            string bandLower = bandName.ToLower();

            // Intelligent chirp configuration based on threat type
            if (bandLower.Contains("dji"))
            {
                // DJI often uses lower portion of 5.8 GHz band
                startRatio = 0.0;
                endRatio = 0.7;
                chirpType = "Focused Low";
            }
            else if (bandLower.Contains("autel"))
            {
                // Autel uses different portion
                startRatio = 0.3;
                endRatio = 1.0;
                chirpType = "Focused High";
            }
            else if (bandLower.Contains("wifi"))
            {
                // WiFi channels clustered, use full band
                chirpType = "Full Band";
            }

            string patternKey = bandName + "_CHIRP_" + chirpType + "_4096";

            if (!loadedPatterns.ContainsKey(patternKey))
            {
                Debug.WriteLine("┌─ Generating CHIRP pattern: " + startFreqMHz + " to " + (startFreqMHz + bandwidthMHz) + " MHz");
                Debug.WriteLine("│   Type: " + chirpType + " | Range: " + (startRatio * 100).ToString("F0") + "%-" + (endRatio * 100).ToString("F0") + "%");

                uint[] pattern = patternGen.GenerateChirpPattern(startRatio, endRatio, 4096);
                loadedPatterns[patternKey] = pattern;

                AddSequences(new SequenceLoadPatternMemory(ltTxCommand, null, pattern));

                Debug.WriteLine("│ ✓ Chirp loaded: " + chirpType + " sweep across " + ((endRatio - startRatio) * bandwidthMHz).ToString("F1") + " MHz");
            }
            else
            {
            ...
            if (dgvBands.Columns.Contains("colStatus"))
            {
                row.Cells["colStatus"].Value = "CHIRP READY (" + chirpType + ")";
                dgvBands.Refresh();
            }

            AddSequences(new SequenceTriggerPattern(ltTxCommand, null));

            Debug.WriteLine("│ Chirp: " + ((endRatio - startRatio) * bandwidthMHz).ToString("F1") + " MHz sweep | Time: 22.76 μs");
        ...
        /// <summary>
        /// ADVANCED: Execute Multi-Tone Pattern Mode with configurable tone density
        /// </summary>
        private void ExecuteMultiToneMode(DataGridViewRow row, string bandName, double startFreqMHz, double bandwidthMHz, int dacChannel)
        {
            ...
            // ADVANCED: Read tone count from grid, or auto-calculate
            int numTones = CalculateOptimalTones(row, bandName, bandwidthMHz);

            string patternKey = bandName + "_MULTITONE_" + numTones + "_4096";

            if (!loadedPatterns.ContainsKey(patternKey))
            {
                Debug.WriteLine("┌─ Generating MULTI-TONE pattern: " + bandName);
                Debug.WriteLine("│   Tones: " + numTones + " | Spacing: " + (bandwidthMHz / numTones).ToString("F2") + " MHz");
                ...
                Debug.WriteLine("│   Power per tone: " + (4096 / numTones) + " samples");
            }
            else
            {
            ...
            if (dgvBands.Columns.Contains("colStatus"))
            {
                row.Cells["colStatus"].Value = "MULTI-TONE READY (" + numTones + ")";
                dgvBands.Refresh();
            }

            AddSequences(new SequenceTriggerPattern(ltTxCommand, null));

            Debug.WriteLine("│ Coverage: " + bandwidthMHz + " MHz | Tone spacing: " + (bandwidthMHz / numTones).ToString("F2") + " MHz");
                ...
                try
                {
                    using (StreamWriter sw = new StreamWriter(sfd.FileName))
                    {
                        ...
                        foreach (DataGridViewRow row in dgvBands.Rows)
                        {
                            ...
                            string stepKhz = "100";
                            string loop = "False";
                            string tones = "8";  // NEW: Default tones
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    foreach (string line in lines)
                    {
                        ...
                        if (data.Length >= 6)
                        {
                            string tones = data.Length >= 7 ? data[6] : "8";  // NEW: Default to 8 if not present
                            dgvBands.Rows.Add("", data[1], data[0], data[2], data[3], data[4], Convert.ToBoolean(data[5]), tones);
                            loadedBands++;
                        }
                    }
        ...
        {
            string features = "DMS Control v3.2 - ADVANCED\n\n" +
                "RF Jamming System Control Software\n" +
                "K9 Electronics Ltd\n\n" +
                "╔══════════════════════════════════════╗\n" +
                "║    ADVANCED AWJ CAPABILITIES         ║\n" +
                "╚══════════════════════════════════════╝\n\n" +
                "✓ Pattern Memory Mode\n" +
                ...
                "  • PRBS (Pseudo-Random)\n" +
                "  • Chirp (Linear Sweep)\n" +
                "  • Multi-Tone (Configurable)\n\n" +
                "✓ ADVANCED: Configurable Tones\n" +
                "  • Auto-detection by threat type\n" +
                "  • Manual override per band\n" +
                "  • 2-64 tones range\n\n" +
                "✓ ADVANCED: Smart Chirp\n" +
                "  • Threat-optimized sweep ranges\n" +
                "  • DJI, Autel targeting\n\n" +
                "✓ Preset Library\n" +
            ...
            MessageBox.Show(features, "About DMS Control v3.2 Advanced", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
