...
namespace dms_control_v3
{
    ...
    public partial class FmMain : Form
    {
        ...
        private int hopDelayMs = 5;  // Total trigger+settle delay per hop (ms)
        ...
        private void tlspbtConnect_Click(object sender, EventArgs e)
        {
            try
            {
                if (usbConnection == null || usbConnection.IsOpen == false)
                {
                    ...
                    this.Text = "AWJ v3.0 — " + String.Format("{0:s}", usbConnection.Info.ProductString);
        ...
        private void CreateGuardBandSettingsBox()
        {
            ...
            lblDwellMs.Text = "ms";
            lblDwellMs.Location = new Point(420, 57);
        ...
        private void ReconfigureDDSBandParams(BandConfiguration band)
        {
            try
            {
                // Use setRampBandwidth — properly calculates DDS parameters
                // within AD9106 constraints (replaces raw math)
                double bwHz = band.BandwidthMHz * 1e6;
                device.dds.setRampBandwidth(bwHz);

                // Read back constrained values
                float freqCtrl = (float)(device.dds.getRampBandwidth() / 10);
                if (freqCtrl < 100000) freqCtrl = 100000;
                device.dds.FreqCtrl = freqCtrl;

                AddSequences(new SequenceSetSweepDDS(ltTxCommand, null,
                    (ushort)AD9106_MODE.RAMP, device.dds.Start, device.dds.Step, device.dds.Points));
                AddSequences(new SequenceSetDDSCtrlFrequency(ltTxCommand, null, freqCtrl));

                // Re-latch AD9106 registers for new parameters
                AddSequences(new SequenceSetRegisterDDS(ltTxCommand, null, 0x1E, 0x0003));
                AddSequences(new SequenceSetRegisterDDS(ltTxCommand, null, 0x1F, 0x0000));
                AddSequences(new SequenceSetRegisterDDS(ltTxCommand, null, 0x1D, 0x0001));

                WaitForCommandQueue(200);  // Reduced from 1000ms for faster band switching
                double achievedBwMHz = device.dds.getRampBandwidth() / 1e6;
                UpdateLiveDdsDisplay(achievedBwMHz, (uint)device.dds.Points, freqCtrl);
            }
            catch (Exception ex)
            {
        ...
        private void btnGuardStart_Click(object sender, EventArgs e)
        {
            ...
            // Safety: if isJamming stuck true from a crashed thread, force reset
            if (isJamming)
            {
                isJamming = false;
                SetControlsJammingMode(false);
                ...
                btnGuardStop.Enabled = false;
                Debug.WriteLine("│ ⚠ Force-reset stuck jamming state");
                return;  // First click resets, second click starts
            }
        ...
        private void btnGuardStop_Click(object sender, EventArgs e)
        {
            ...
            // Force-reset UI state regardless of thread status
            try
            {
        ...
        private void ExecuteGuardBandJamming()
        {
            ...
            finally
            {
                ...
                catch { }  // Form may be closing — don't crash
            }
        }
        ...
        private void ConfigureDDSForJamming(BandConfiguration firstBand)
        {
            try
            {
                ...
                // Use setRampBandwidth — this is the proven path that correctly
                // calculates DDS Start/Step/Points within AD9106 constraints.
                // Raw math (bwHz/2, bwHz/points) produces values the hardware can't use.
                double bwHz = firstBand.BandwidthMHz * 1e6;
                device.dds.setRampBandwidth(bwHz);

                // Read back the constrained values that setRampBandwidth computed
                uint points = (uint)device.dds.Points;
                device.dds.FreqCtrl = (float)(device.dds.getRampBandwidth() / 10);
                if (device.dds.FreqCtrl < 100000) device.dds.FreqCtrl = 100000;  // Min 100 kHz

                // Send DDS sweep parameters
                AddSequences(new SequenceSetSweepDDS(ltTxCommand, null,
                    ddsMode, device.dds.Start, device.dds.Step, device.dds.Points));
                ...
                // Latch configuration and trigger AD9106
                AddSequences(new SequenceSetRegisterDDS(ltTxCommand, null, 0x1E, 0x0003));
                AddSequences(new SequenceSetRegisterDDS(ltTxCommand, null, 0x1F, 0x0000));
                AddSequences(new SequenceSetRegisterDDS(ltTxCommand, null, 0x1D, 0x0001));
                ...
                // Update live DDS display with actual achieved bandwidth
                double achievedBwMHz = device.dds.getRampBandwidth() / 1e6;
        ...
        private void HopToFrequency(double frequencyMHz, double attenDb = -1)
        {
            try
            {
                ...
                // hopDelayMs is adjustable from UI "Hop:" spinner (default 5ms)
                int triggerMs = Math.Max(hopDelayMs, 1);
                int settleMs = Math.Max(triggerMs / 2, 1);
                AddSequences(new SequenceSetDDSCtrlFrequency(ltTxCommand, null, 10));
                AddSequences(new SequenceDelay(triggerMs));
                AddSequences(new SequenceSetDDSCtrlFrequency(ltTxCommand, null, 0));
                AddSequences(new SequenceDelay(settleMs));
                ...
                WaitForCommandQueue(200);  // Fixed safe value — hop delay only affects DDS trigger timing
        ...
        private void ShowJammingStatusWindow()
        {
            ...
            jammingStatusForm.Size = new Size(380, 120);
            jammingStatusForm.FormBorderStyle = FormBorderStyle.FixedToolWindow;
            jammingStatusForm.StartPosition = FormStartPosition.Manual;
            jammingStatusForm.TopMost = true;
            jammingStatusForm.BackColor = Color.FromArgb(25, 30, 20);
            jammingStatusForm.ShowInTaskbar = false;

            // Position: top-right of main form
            jammingStatusForm.Location = new Point(
                this.Location.X + this.Width - jammingStatusForm.Width - 10,
                this.Location.Y + 30);

            // Band info label (large)
            lblJammingStatusText = new Label();
            lblJammingStatusText.Font = new Font("Consolas", 12F, FontStyle.Bold);
            lblJammingStatusText.ForeColor = Color.FromArgb(255, 80, 80);
            lblJammingStatusText.BackColor = Color.FromArgb(35, 40, 30);
            lblJammingStatusText.Location = new Point(5, 8);
            lblJammingStatusText.Size = new Size(360, 32);
            ...
            lblJammingCycleText.Location = new Point(5, 46);
            lblJammingCycleText.Size = new Size(360, 28);
            ...
            // Don't block main form if closed — just hide during jamming
