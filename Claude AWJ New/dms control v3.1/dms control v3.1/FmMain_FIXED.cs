...
namespace dms_control_v3
{
    ...
    public partial class FmMain : Form
    {
        ...
        #endregion
        private int k9ExtendedMode = 0;  // Tracks UI mode index (0-6) vs hardware AD9106_MODE (0-3)
        ...
        public FmMain(GlobalConfigurations _cfg)
        {
            ...
            if (cmbbxDdsMode != null)
            {
                ...
                cmbbxDdsMode.Items.AddRange(new object[] {
                    ...
                    "Guard Band",   // 5 → AD9106 RAMP + guard band hopping
                    "TDM Jam"       // 6 → AD9106 RAMP + TDM fast hop
                });
        ...
        private void cbfuncInitDeviceList(params object[] data)
        {
            ...
            else
            {
                ...
                // Read TDM standalone config from EEPROM
                tdmEepromValid = false;
                for (int i = 0; i < 8; i++) tdmEepromBandValid[i] = false;

                // Header at 448 (18 bytes: 16 data + 2 CRC)
                AddSequences(new SequenceReadMemory(ltTxCommand, cdfuncReadConfiguration, TDM_EEPROM_HEADER, 18));
                AddSequences(new SequenceCallFunction(cbfuncIncrementStatusBar, null)); cmd_counter++;

                // 8 bands at 480 + i*32 (30 bytes each: 28 data + 2 CRC)
                for (int i = 0; i < 8; i++)
                {
                    AddSequences(new SequenceReadMemory(ltTxCommand, cdfuncReadConfiguration,
                        (ushort)(TDM_EEPROM_BANDS + (i * 32)), 30));
                    AddSequences(new SequenceCallFunction(cbfuncIncrementStatusBar, null)); cmd_counter++;
                }
            ...
            else
            {
                ...
                if (tdmEepromValid && tdmEepromNumBands > 0)
                {
                    try
                    {
                        ...
                        if (numTdmDwell != null && tdmEepromDwellMs >= 1 && tdmEepromDwellMs <= 500)
        ...
        private void LoParameters_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                ...
                if (e.KeyChar == 13)
                {
                    ...
                    ushort lo_cmd_counter = 0;

                    // K9 FIX: Send sweep parameters to firmware
                    ...
                    AddSequences(new SequenceCallFunction(cbfuncIncrementStatusBar, null)); lo_cmd_counter++;

                    // K9 FIX: Write all 6 PLL registers to force MAX2871 to reprogram immediately
                    // Without this, the new frequency sits in firmware memory but MAX2871 is never told
                    for (int i = 0; i < 6; i++)
                    {
                        AddSequences(new SequenceWriteRegisterPLL(ltTxCommand, null, (ulong)device.lo.pll.registers[i].body));
                        AddSequences(new SequenceCallFunction(cbfuncIncrementStatusBar, null)); lo_cmd_counter++;
                    ...
                    // K9 FIX: Trigger firmware sweep loop to apply the new LO frequency
                    // This DDS ctrl "kick" is the same pattern used by SetDeviceConfiguration()
                    // which works correctly - without it the PLL params are stored but not activated
                    AddSequences(new SequenceSetDDSCtrlFrequency(ltTxCommand, null, (float)device.dds.FreqCtrl));
                    AddSequences(new SequenceCallFunction(cbfuncIncrementStatusBar, null)); lo_cmd_counter++;

                    // K9 FIX: Set up progress bar (was completely missing - 8 steps total)
                    tlspStatusPanel.Maximum = lo_cmd_counter;
                    tlspStatusPanel.Value = 0;
                }
                else
                {
        ...
        private void cmbbxDdsMode_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                switch (selectedIdx)
                {
                    ...
                    case 5: hwMode = (ushort)AD9106_MODE.RAMP; break;   // Guard Band (RAMP + LO hopping)
                    case 6: hwMode = (ushort)AD9106_MODE.RAMP; break;   // TDM Jam (RAMP + fast TDM)
        ...
        private void CreateGuardBandSettingsBox()
        {
            ...
            btnLoad7Bands.Font = new Font("Segoe UI", 7.5F, FontStyle.Bold);
            ...
            btnLoad7Bands.Size = new Size(100, 26);
            ...
            btnLoad4Bands.Font = new Font("Segoe UI", 7.5F, FontStyle.Bold);
            ...
            btnLoad4Bands.Size = new Size(90, 26);
            btnLoad4Bands.Location = new Point(113, 18);
            ...
            btnLoadGps.Font = new Font("Segoe UI", 7.5F, FontStyle.Bold);
            ...
            btnLoadGps.Size = new Size(90, 26);
            btnLoadGps.Location = new Point(208, 18);
            ...
            btnClearTable.Font = new Font("Segoe UI", 7.5F, FontStyle.Bold);
            ...
            btnClearTable.Size = new Size(60, 26);
            btnClearTable.Location = new Point(303, 18);
            ...
            chkGuardPulseMode.Font = new Font("Segoe UI", 7.5F, FontStyle.Bold);
            chkGuardPulseMode.ForeColor = Color.FromArgb(150, 220, 120);
            chkGuardPulseMode.Location = new Point(8, 50);
            chkGuardPulseMode.Size = new Size(90, 18);
            ...
            lblOn.Text = "ON:";
            lblOn.Font = new Font("Segoe UI", 7F);
            lblOn.ForeColor = Color.FromArgb(180, 200, 160);
            lblOn.Location = new Point(100, 51);
            lblOn.Size = new Size(25, 16);
            ...
            numGuardPulseOn.Width = 45;
            numGuardPulseOn.Location = new Point(125, 48);
            numGuardPulseOn.Font = new Font("Segoe UI", 7F);
            ...
            lblOnMs.Text = "ms";
            lblOnMs.Location = new Point(172, 51);
            lblOnMs.Size = new Size(18, 16);
            lblOnMs.Font = new Font("Segoe UI", 6.5F);
            lblOnMs.ForeColor = Color.FromArgb(150, 170, 140);
            grpGuardBandSettings.Controls.Add(lblOnMs);

            Label lblOff = new Label();
            lblOff.Text = "OFF:";
            lblOff.Font = new Font("Segoe UI", 7F);
            lblOff.ForeColor = Color.FromArgb(180, 200, 160);
            lblOff.Location = new Point(192, 51);
            lblOff.Size = new Size(27, 16);
            ...
            numGuardPulseOff.Width = 45;
            numGuardPulseOff.Location = new Point(220, 48);
            numGuardPulseOff.Font = new Font("Segoe UI", 7F);
            ...
            lblOffMs.Text = "ms";
            lblOffMs.Location = new Point(267, 51);
            lblOffMs.Size = new Size(18, 16);
            lblOffMs.Font = new Font("Segoe UI", 6.5F);
            lblOffMs.ForeColor = Color.FromArgb(150, 170, 140);
            grpGuardBandSettings.Controls.Add(lblOffMs);

            // Dwell (moved from standalone panel)
            Label lblDwell = new Label();
            lblDwell.Text = "Dwell:";
            lblDwell.Font = new Font("Segoe UI", 7F);
            lblDwell.ForeColor = Color.FromArgb(180, 200, 160);
            lblDwell.Location = new Point(295, 51);
            lblDwell.Size = new Size(35, 16);
            ...
            numTdmDwell.Value = 5;
            numTdmDwell.Width = 45;
            numTdmDwell.Location = new Point(330, 48);
            numTdmDwell.Font = new Font("Segoe UI", 7F);
            ...
            lblDwellMs.Text = "ms";
            lblDwellMs.Location = new Point(377, 51);
            lblDwellMs.Size = new Size(18, 16);
            lblDwellMs.Font = new Font("Segoe UI", 6.5F);
            ...
            lblGuardStatus.Font = new Font("Consolas", 8F, FontStyle.Bold);
            lblGuardStatus.ForeColor = Color.FromArgb(100, 255, 100);
            lblGuardStatus.Location = new Point(8, 72);
            lblGuardStatus.Size = new Size(w - 20, 28);
            lblGuardStatus.BorderStyle = BorderStyle.FixedSingle;
            lblGuardStatus.BackColor = Color.FromArgb(30, 35, 25);
            ...
            btnTdmUpload.Size = new Size(140, 32);
            btnTdmUpload.Location = new Point(8, 105);
            btnTdmUpload.Click += btnSaveToDevice_Click;
            ...
            btnGuardStart.Size = new Size(80, 32);
            btnGuardStart.Location = new Point(155, 105);
            ...
            btnGuardStop.Size = new Size(80, 32);
            btnGuardStop.Location = new Point(240, 105);
            ...
            btnDiag.Text = "DIAG";
            btnDiag.Font = new Font("Segoe UI", 7.5F, FontStyle.Bold);
            ...
            btnDiag.Size = new Size(55, 32);
            btnDiag.Location = new Point(325, 105);
            ...
            ledConnection.Location = new Point(395, 112);
            ...
            lblConnectionStatus.Text = "OFFLINE";
            lblConnectionStatus.Font = new Font("Segoe UI", 6.5F, FontStyle.Bold);
            lblConnectionStatus.ForeColor = Color.FromArgb(255, 80, 80);
            lblConnectionStatus.Location = new Point(409, 113);
            lblConnectionStatus.Size = new Size(50, 12);
            grpGuardBandSettings.Controls.Add(lblConnectionStatus);

            // Workflow hint
            Label lblHint = new Label();
            lblHint.Text = "Configure bands \u2192 Save to Device \u2192 auto-starts on power-up (no PC needed)";
            lblHint.Font = new Font("Segoe UI", 6.5F, FontStyle.Italic);
            lblHint.ForeColor = Color.FromArgb(120, 140, 110);
            lblHint.Location = new Point(8, 140);
            lblHint.Size = new Size(w - 20, 13);
            grpGuardBandSettings.Controls.Add(lblHint);
        ...
        // TDM USB commands for START/STOP only (bulk upload uses EEPROM writes)
        ...
        private void btnSaveToDevice_Click(object sender, EventArgs e)
        {
            ...
            var result = MessageBox.Show(
                "Save full configuration to device EEPROM?\n\n" +
                "This writes:\n" +
                "  \u2022 PLL / DDS / attenuator settings\n" +
                "  \u2022 TDM band table + timing\n\n" +
                "The board will auto-start jamming on power-up\n" +
            ...
            try
            {
                // ── STEP 1: Save hardware config (PLL, DDS, attenuators) ──
                // This is the same as the Configuration → Save menu item
                tlspbtConfigSave_Click(sender, e);
                Thread.Sleep(300);  // Let EEPROM writes complete

                // ── STEP 2: Stop any running TDM ──
                int cmdSize = Command.GetCommandSize();
                byte[] stopPacket = new byte[cmdSize];
                ...
                // ── STEP 3: Build and save TDM band table ──
                byte[] write_data = new byte[64];
                ushort len;
                ushort crc;
                byte[] tmpBuff;
                ...
                List<byte[]> bandPages = new List<byte[]>();
                ...
                foreach (DataGridViewRow row in dgvBands.Rows)
                {
                    ...
                    // Read DDS mode (Ramp or PRBS)
                    byte ddsModeByte = 0;
                    if (dgvBands.Columns.Contains("colDdsMode") && row.Cells["colDdsMode"].Value != null)
                    {
                        if (row.Cells["colDdsMode"].Value.ToString() == "PRBS") ddsModeByte = 1;
                    }

                    // Read per-band attenuation (0-25 dB)
                    double attenDb = 0.0;
                    if (dgvBands.Columns.Contains("colAttenDb") && row.Cells["colAttenDb"].Value != null)
                        try { attenDb = Convert.ToDouble(row.Cells["colAttenDb"].Value); } catch { }
                    byte attenByte = (byte)Math.Min(250, Math.Max(0, (int)(attenDb * 10)));

                    // DDS parameter calculations
                    ...
                    {
                        ddsPoints = (uint)Math.Max(tones * 16, 32);
                        ddsStep = ddsBwHz / ddsPoints;
                    }

                    // Build tsTdmBand struct (32 bytes)
                    // lo_frequency(8) + dds_bandwidth(4) + dds_step(4) +
                    // dds_points(4) + dds_ctrl_freq(4) + active(1) + dds_mode(1) + atten_x10(1) + reserved(1)
                    byte[] bandData = new byte[32];
                    Array.Copy(BitConverter.GetBytes(loFreqHz), 0, bandData, 0, 8);
                    Array.Copy(BitConverter.GetBytes(ddsBwHz), 0, bandData, 8, 4);
                    Array.Copy(BitConverter.GetBytes(ddsStep), 0, bandData, 12, 4);
                    Array.Copy(BitConverter.GetBytes(ddsPoints), 0, bandData, 16, 4);
                    Array.Copy(BitConverter.GetBytes(ddsCtrlFreq), 0, bandData, 20, 4);
                    bandData[24] = (byte)(active ? 1 : 0);
                    bandData[25] = ddsModeByte;
                    bandData[26] = attenByte;
                    bandData[27] = 0;

                    crc = nsAlexKir.MathLibrary.CRC.CalculateCRC16(bandData, 28);
                    bandData[28] = (byte)(crc & 0xFF);
                    bandData[29] = (byte)((crc >> 8) & 0xFF);

                    bandPages.Add(bandData);
                    bandCount++;

                    Debug.WriteLine(string.Format("SAVE band {0}: {1} MHz, BW={2}, Mode={3}, Atten={4}dB",
                        bandCount, freqMhz, bwMhz, ddsModeByte == 1 ? "PRBS" : "Ramp", attenDb));
                }
                ...
                // ── Write TDM header to EEPROM address 448 ──
                len = 0;
                Array.Clear(write_data, 0, write_data.Length);
                write_data[len++] = (byte)(pulseMode ? 2 : 1);
                write_data[len++] = (byte)bandCount;
                write_data[len++] = (byte)(dwellMs & 0xFF);
                write_data[len++] = (byte)((dwellMs >> 8) & 0xFF);
                write_data[len++] = (byte)(pulseOn & 0xFF);
                write_data[len++] = (byte)((pulseOn >> 8) & 0xFF);
                write_data[len++] = (byte)(pulseOff & 0xFF);
                write_data[len++] = (byte)((pulseOff >> 8) & 0xFF);
                write_data[len++] = 0x4B;  // 'K' signature
                write_data[len++] = 0x39;  // '9' signature
                while (len < 16) write_data[len++] = 0;
                crc = nsAlexKir.MathLibrary.CRC.CalculateCRC16(write_data, 16);
                write_data[len++] = (byte)(crc & 0xFF);
                write_data[len++] = (byte)((crc >> 8) & 0xFF);

                AddSequences(new SequenceWriteMemory(ltTxCommand, null,
                    TDM_EEPROM_HEADER, write_data, len));
                AddSequences(new SequenceDelay(100));

                // ── Write each band to EEPROM ──
                for (int i = 0; i < bandPages.Count; i++)
                {
                    ushort bandAddr = (ushort)(TDM_EEPROM_BANDS + (i * 32));
                    AddSequences(new SequenceWriteMemory(ltTxCommand, null,
                        bandAddr, bandPages[i], 30));
                    AddSequences(new SequenceDelay(100));
                }

                // Clear unused band slots
                for (int i = bandPages.Count; i < 8; i++)
                {
                    byte[] emptyBand = new byte[32];
                    crc = nsAlexKir.MathLibrary.CRC.CalculateCRC16(emptyBand, 28);
                    emptyBand[28] = (byte)(crc & 0xFF);
                    emptyBand[29] = (byte)((crc >> 8) & 0xFF);
                    ushort bandAddr = (ushort)(TDM_EEPROM_BANDS + (i * 32));
                    AddSequences(new SequenceWriteMemory(ltTxCommand, null,
                        bandAddr, emptyBand, 30));
                    AddSequences(new SequenceDelay(100));
                }
                ...
                {
                    lblGuardStatus.Text = string.Format(
                        " SAVED: {0} bands | {1}ms dwell | {2}",
                        bandCount, dwellMs, pulseMode ? "PULSED" : "CONTINUOUS");
                    lblGuardStatus.ForeColor = Color.FromArgb(100, 255, 100);
                }

                MessageBox.Show(string.Format(
                    "Full configuration saved to EEPROM:\n\n" +
                    "  \u2022 {0} bands\n" +
                    "  \u2022 Dwell: {1}ms\n" +
                    "  \u2022 Mode: {2}\n" +
                    "  \u2022 PLL / DDS / Attenuators: saved\n\n" +
                    "Power cycle the board \u2014 jamming will\n" +
                    "start automatically (no PC needed).",
                    bandCount, dwellMs,
                    pulseMode ? string.Format("Pulsed ({0}ms on / {1}ms off)", pulseOn, pulseOff) : "Continuous"),
                    "Configuration Saved",
        ...
        private void btnTdmStart_Click(object sender, EventArgs e)
        {
            ...
            // Send single CMD_TDM_START via sequence system
            int cmdSize = Command.GetCommandSize();
                ...
                if (usePulseMode)
                {
                    ...
                    for (int cycle = 0; cycle < pulseCycles && isJamming; cycle++)
                    {
                        ...
                        for (int i = 0; i < allBands.Count && isJamming; i++)
                        {
                            ...
                            else if (!allBands[i].IsPRBS &&
                                (allBands[i].BandwidthMHz != currentBW || allBands[i].Tones != currentTones))
                            {
                    ...
                    while (isJamming)
                    {
                        for (int i = 0; i < allBands.Count && isJamming; i++)
                        {
                            ...
                            if (allBands[i].DdsMode != currentMode_c)
                            {
                                ...
                                (allBands[i].BandwidthMHz != currentBW_c || allBands[i].Tones != currentTones_c))
                            {
                                ReconfigureDDSBandParams(allBands[i]);
                                currentBW_c = allBands[i].BandwidthMHz;
                                currentTones_c = allBands[i].Tones;
        ...
        private void UpdateTDMStatus(int cycle, int currentBatch, int totalBatches, bool jamming)
        {
            this.Invoke((MethodInvoker)delegate
            {
                ...
                if (jamming)
                {
                    ...
                    lblGuardStatus.Text = string.Format(" JAMMING: {0} | {1}",
                        batchInfo, cycleStr);
                ...
                {
                    string cycleStr = pulseCycles == int.MaxValue ? "Test " + (cycle + 1) : "Pulse " + (cycle + 1) + "/" + pulseCycles;
                    lblGuardStatus.Text = string.Format(" SILENT | {0}",
                        cycleStr);
                    lblGuardStatus.ForeColor = Color.FromArgb(100, 255, 100);
                }
            });
