...
namespace dms_control_v3
{
    ...
    public partial class FmMain : Form
    {
        ...
        private int dwellTimeMs = 5;  // User-adjustable ms per band hop
        ...
        public FmMain(GlobalConfigurations _cfg)
        {
            ...
            // K9: Refresh hits/sec when grid data changes
            if (dgvBands != null)
            {
                dgvBands.RowsAdded += (s, ev) => UpdateGuardStatus();
                dgvBands.RowsRemoved += (s, ev) => UpdateGuardStatus();
                dgvBands.CellValueChanged += (s, ev) =>
                {
                    if (ev.ColumnIndex >= 0 && dgvBands.Columns[ev.ColumnIndex].Name == "colActive")
                        UpdateGuardStatus();
                };
                dgvBands.CurrentCellDirtyStateChanged += (s, ev) =>
                {
                    if (dgvBands.CurrentCell is DataGridViewCheckBoxCell)
        ...
        private void CreateGuardBandSettingsBox()
        {
            ...
            lblDwell.Location = new Point(445, 53);
            lblDwell.Size = new Size(38, 18);
            grpGuardBandSettings.Controls.Add(lblDwell);

            NumericUpDown numDwellMs = new NumericUpDown();
            numDwellMs.Minimum = 1;
            numDwellMs.Maximum = 100;
            numDwellMs.Value = 5;
            numDwellMs.Width = 45;
            numDwellMs.Location = new Point(483, 50);
            numDwellMs.ValueChanged += (s, e) => { dwellTimeMs = (int)numDwellMs.Value; UpdateGuardStatus(); };
            grpGuardBandSettings.Controls.Add(numDwellMs);

            Label lblDwellUnit = new Label();
            lblDwellUnit.Text = "ms";
            lblDwellUnit.Location = new Point(530, 53);
            lblDwellUnit.Size = new Size(22, 18);
            lblDwellUnit.Font = new Font("Segoe UI", 7F);
            grpGuardBandSettings.Controls.Add(lblDwellUnit);
            ...
            lblGuardStatus.Size = new Size(420, 36);
        ...
        private void UpdateGuardStatus()
        {
            ...
            if (chkGuardPulseMode.Checked)
            {
                ...
                int activeBands = CountActiveBands();
                double hitsPerSec = 0;
                if (activeBands > 0 && dwellTimeMs > 0)
                {
                    double cycleTimeMs = dwellTimeMs * activeBands;
                    hitsPerSec = (1000.0 / cycleTimeMs) * (dutyCycle / 100.0);
                }
                lblGuardStatus.Text = string.Format(
                    " {0} bands | {1:F0} hits/sec/band | Dwell: {2}ms\n Duty: {3:F1}% | Duration: {4:F1}min",
                    activeBands, hitsPerSec, dwellTimeMs, dutyCycle, durationMin);
            ...
            {
                int activeBands_c = CountActiveBands();
                double hitsPerSec_c = 0;
                if (activeBands_c > 0 && dwellTimeMs > 0)
                    hitsPerSec_c = 1000.0 / (dwellTimeMs * activeBands_c);
                lblGuardStatus.Text = string.Format(
                    " {0} bands | {1:F0} hits/sec/band | Dwell: {2}ms\n CONTINUOUS - WiFi impact HIGH",
                    activeBands_c, hitsPerSec_c, dwellTimeMs);
                lblGuardStatus.ForeColor = Color.Red;
            }
        }
        ...
        private void WaitForCommandQueue(int timeoutMs = 5000)
        {
            ...
            {
                if (!isJamming) break;  // Allow STOP to interrupt
        ...
        /// <summary>
        /// Lightweight queue throttle for fast hopping.
        /// Only pauses if queue backed up beyond threshold.
        /// </summary>
        private void ThrottleCommandQueue(int maxQueued = 3)
        {
            int waited = 0;
            while (ltTxCommand.Count > maxQueued && waited < 200)
            {
                if (!isJamming) break;
                Thread.Sleep(2);
                waited += 2;
        ...
        private void ConfigureDDSForJamming(BandConfiguration firstBand)
        {
            try
            {
                ...
                // Configure AD9106 registers for RAMP mode
                // Using device.dds.Channels exactly like DDSSetConfiguration RAMP path
                ushort Cycle = 1;
                device.dds.registers[0x20] = 0x0001;
                device.dds.registers[0x26] = (ushort)((device.dds.Channels[3] == true) ? 0x3200 : 0x0000);
                device.dds.registers[0x26] += (ushort)((device.dds.Channels[2] == true) ? 0x0032 : 0x0000);
                ...
                // Start DDS output (matching original DDSSetConfiguration timing)
                AddSequences(new SequenceSetDDSCtrlFrequency(ltTxCommand, null, 10));
                AddSequences(new SequenceDelay(500));
                AddSequences(new SequenceSetDDSCtrlFrequency(ltTxCommand, null, 0));
                AddSequences(new SequenceDelay(100));
                ...
                Thread.Sleep(200);  // Extra settle time for AD9106
        ...
        private void HopToFrequency(double frequencyMHz)
        {
            try
            {
                ...
                // PLL update - throttle only if queue backed up
                AddSequences(new SequenceSetSweepLoParameters(ltTxCommand, null,
                    (float)freqHz, 0, 1, 0, 0, 0));
                ThrottleCommandQueue(3);
        ...
        private void UpdateTDMStatus(int cycle, int currentBatch, int totalBatches, bool jamming)
        {
            this.Invoke((MethodInvoker)delegate
            {
                ...
                // Calculate hits/sec
                double hitsPerSec = 0;
                if (totalBatches > 0 && dwellTimeMs > 0)
                    hitsPerSec = 1000.0 / (dwellTimeMs * totalBatches);

                if (jamming)
                {
                    lblGuardStatus.Text = string.Format(
                        " \u25cf JAMMING: Band {0}/{1} | Dwell: {2}ms\n {3:F0} hits/sec/band | Pulse {4}/{5}",
                        currentBatch + 1, totalBatches, dwellTimeMs,
                        hitsPerSec, cycle + 1, pulseCycles);
                    lblGuardStatus.ForeColor = Color.Red;
                }
                else
                {
                    lblGuardStatus.Text = string.Format(
                        " \u25cb SILENT | Pulse {0}/{1}\n {2:F0} hits/sec/band | WiFi Recovery",
                        cycle + 1, pulseCycles, hitsPerSec);
