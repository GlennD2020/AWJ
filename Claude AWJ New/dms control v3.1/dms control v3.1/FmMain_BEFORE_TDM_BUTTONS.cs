...
namespace dms_control_v3
{
    ...
    public partial class FmMain : Form
    {
        ...
        /// <summary>
        /// Send a raw USB command to the DMS board.
        /// Builds a 32-byte packet matching tsSysCmd: {Type(2), Status(2), Len(2), data[26]}
        /// </summary>
        ...
        /// <summary>
        /// Create the TDM Standalone Control panel below Guard Band Controls.
        /// Provides Upload/Save/Load/Start/Stop for onboard standalone operation.
        /// </summary>
        private void CreateTdmStandalonePanel()
        {
            ...
            lblInfo.Text = "Upload bands \u2192 Save EEPROM \u2192 device runs standalone after power cycle";
        ...
        // ═══════════════════════════════════════════════════════════
        //  UPLOAD BANDS TO BOARD
        //  Reads all active bands from the grid, converts to firmware
        //  format, sends CMD_SET_TDM_BAND (40) for each, then
        //  CMD_SET_TDM_PARAMS (42) for dwell/pulse settings.
        // ═══════════════════════════════════════════════════════════
        private void btnTdmUpload_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dgvBands.Rows)
                {
                    ...
                    // Calculate DDS parameters (same formulas as ConfigureDDSForJamming)
                    ...
                    // [0]=index, [1..8]=lo_freq(double), [9..12]=bw(float),
                    // [13..16]=step(float), [17..20]=points(uint32),
                    // [21..24]=ctrl_freq(float), [25]=active
                    ...
                    SendTdmCommand(CMD_SET_TDM_BAND, data, 26);
                    Thread.Sleep(20);

                    bandCount++;
                    Debug.WriteLine(string.Format("TDM Upload band {0}: {1} MHz, BW={2} MHz, Tones={3}",
                ...
                {
                    lblTdmStatus.Text = string.Format(" Uploaded {0} bands, dwell={1}ms", bandCount, dwellMs);
        ...
        // ═══════════════════════════════════════════════════════════
        //  SAVE TO EEPROM - persists band table for standalone boot
        // ═══════════════════════════════════════════════════════════
        private void btnTdmSaveEeprom_Click(object sender, EventArgs e)
        {
            ...
            var result = MessageBox.Show(
                "Save current TDM band table to EEPROM?\n\n" +
            ...
            if (result == DialogResult.Yes)
            {
                ...
                if (lblTdmStatus != null)
                {
                    lblTdmStatus.Text = " EEPROM SAVED - auto-loads on boot";
                    lblTdmStatus.ForeColor = Color.DarkGreen;
                }
                Debug.WriteLine("TDM: Save to EEPROM sent");
            }
        }

        // ═══════════════════════════════════════════════════════════
        //  LOAD FROM EEPROM - reload saved config into RAM
        // ═══════════════════════════════════════════════════════════
        ...
        private void btnTdmStart_Click(object sender, EventArgs e)
        {
            ...
            Debug.WriteLine("TDM: Start command sent");
        }
        ...
        private void btnTdmStop_Click(object sender, EventArgs e)
        {
            ...
            Debug.WriteLine("TDM: Stop command sent");
        }
