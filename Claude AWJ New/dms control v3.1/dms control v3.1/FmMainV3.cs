    ...
    public partial class FmMain : Form
    {
        ...
        public FmMain(GlobalConfigurations _cfg)
        {
            ...
            if (dgvBands != null)
            {
                ...
                // - colStepKhz: step is auto-calculated from BW/Points (via Tones)
                // - Loop column: TDM always loops all active bands automatically
                foreach (DataGridViewColumn col in dgvBands.Columns)
                {
                    // Hide by name
                    if (col.Name == "colBwMhz" || col.Name == "colStepKhz")
                        col.Visible = false;
                    // Hide Loop column (might be named differently by Designer)
                    if (col.HeaderText != null && col.HeaderText.Trim() == "Loop")
                        col.Visible = false;
                }
        ...
        private void CreateGuardBandSettingsBox()
        {
            ...
            btnClearTable.BackColor = Color.FromArgb(120, 120, 130);
            btnClearTable.ForeColor = Color.White;
            btnClearTable.FlatStyle = FlatStyle.Flat;
            btnClearTable.Size = new Size(110, 28);
            btnClearTable.Location = new Point(322, 18);
            btnClearTable.Click += (s, e) =>
            {
                if (dgvBands != null)
                {
                    ...
                    Debug.WriteLine("║ TABLE CLEARED - ready for manual entry ║");
                }
            };
        ...
        // TDM USB Command IDs (must match firmware dms_tdm.h)
        ...
        /// <summary>
        /// Send a raw TDM USB command.
        /// Builds 32-byte tsSysCmd: {Type(2), Status(2), Len(2), data[26]}
        /// Uses lock for thread safety with USB write thread.
        /// </summary>
        private void SendTdmCommand(short cmdType, byte[] data, int dataLen)
        {
            ...
            {
                int copyLen = Math.Min(dataLen, 26);
                Array.Copy(data, 0, packet, 6, copyLen);
            }
            lock (ltTxCommand)
            {
                ltTxCommand.Add(new Command(packet, Command.GetCommandSize()));
            }
        }

        /// <summary>
        /// Wait until ltTxCommand is drained by USB thread, with timeout.
        /// </summary>
        private void WaitForTdmQueueDrain(int timeoutMs = 3000)
        {
            Stopwatch sw = Stopwatch.StartNew();
            while (sw.ElapsedMilliseconds < timeoutMs)
            {
                if (ltTxCommand.Count == 0) return;
                Thread.Sleep(10);
            }
        }

        /// <summary>
        /// Create TDM Standalone Control panel below Guard Band Controls.
        /// </summary>
        private void CreateTdmStandalonePanel()
        {
            if (grpGuardBandSettings == null) return;

            int x = grpGuardBandSettings.Left;
            int y = grpGuardBandSettings.Bottom + 5;
            int w = grpGuardBandSettings.Width;

            grpTdmStandalone = new GroupBox();
            grpTdmStandalone.Text = " STANDALONE TDM (ONBOARD) ";
            grpTdmStandalone.Font = new Font("Segoe UI", 8F, FontStyle.Bold);
            grpTdmStandalone.ForeColor = Color.DarkRed;
            grpTdmStandalone.Location = new Point(x, y);
            grpTdmStandalone.Size = new Size(w, 105);
            grpTdmStandalone.BackColor = Color.FromArgb(245, 235, 230);
            grpTdmStandalone.Anchor = AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Bottom;
            this.Controls.Add(grpTdmStandalone);
            grpTdmStandalone.BringToFront();

            // Row 1: Upload + Save + Load + Dwell
            btnTdmUpload = new Button();
            btnTdmUpload.Text = "UPLOAD TO BOARD";
            btnTdmUpload.Font = new Font("Segoe UI", 8F, FontStyle.Bold);
            btnTdmUpload.BackColor = Color.FromArgb(0, 100, 180);
            btnTdmUpload.ForeColor = Color.White;
            btnTdmUpload.FlatStyle = FlatStyle.Flat;
            btnTdmUpload.Size = new Size(130, 28);
            btnTdmUpload.Location = new Point(8, 18);
            btnTdmUpload.Click += btnTdmUpload_Click;
            grpTdmStandalone.Controls.Add(btnTdmUpload);

            btnTdmSaveEeprom = new Button();
            btnTdmSaveEeprom.Text = "SAVE TO EEPROM";
            btnTdmSaveEeprom.Font = new Font("Segoe UI", 8F, FontStyle.Bold);
            btnTdmSaveEeprom.BackColor = Color.FromArgb(180, 80, 0);
            btnTdmSaveEeprom.ForeColor = Color.White;
            btnTdmSaveEeprom.FlatStyle = FlatStyle.Flat;
            btnTdmSaveEeprom.Size = new Size(125, 28);
            btnTdmSaveEeprom.Location = new Point(143, 18);
            btnTdmSaveEeprom.Click += btnTdmSaveEeprom_Click;
            grpTdmStandalone.Controls.Add(btnTdmSaveEeprom);

            btnTdmLoadEeprom = new Button();
            btnTdmLoadEeprom.Text = "LOAD EEPROM";
            btnTdmLoadEeprom.Font = new Font("Segoe UI", 8F, FontStyle.Bold);
            btnTdmLoadEeprom.BackColor = Color.FromArgb(100, 100, 120);
            btnTdmLoadEeprom.ForeColor = Color.White;
            btnTdmLoadEeprom.FlatStyle = FlatStyle.Flat;
            btnTdmLoadEeprom.Size = new Size(110, 28);
            btnTdmLoadEeprom.Location = new Point(273, 18);
            btnTdmLoadEeprom.Click += btnTdmLoadEeprom_Click;
            grpTdmStandalone.Controls.Add(btnTdmLoadEeprom);

            Label lblDwell = new Label();
            lblDwell.Text = "Dwell:";
            lblDwell.Font = new Font("Segoe UI", 8F);
            lblDwell.Location = new Point(390, 22);
            lblDwell.Size = new Size(38, 18);
            grpTdmStandalone.Controls.Add(lblDwell);

            numTdmDwell = new NumericUpDown();
            numTdmDwell.Minimum = 1;
            numTdmDwell.Maximum = 500;
            numTdmDwell.Value = 5;
            numTdmDwell.Width = 50;
            numTdmDwell.Location = new Point(428, 19);
            numTdmDwell.Font = new Font("Segoe UI", 8F);
            numTdmDwell.ValueChanged += (s2, e2) => UpdateTdmHitsDisplay();
            grpTdmStandalone.Controls.Add(numTdmDwell);

            Label lblDwellMs = new Label();
            lblDwellMs.Text = "ms";
            lblDwellMs.Font = new Font("Segoe UI", 7F);
            lblDwellMs.Location = new Point(480, 22);
            lblDwellMs.Size = new Size(20, 16);
            grpTdmStandalone.Controls.Add(lblDwellMs);

            // Row 2: Start + Stop + Status
            btnTdmStart = new Button();
            btnTdmStart.Text = "START TDM";
            btnTdmStart.Font = new Font("Segoe UI", 9F, FontStyle.Bold);
            btnTdmStart.BackColor = Color.Green;
            btnTdmStart.ForeColor = Color.White;
            btnTdmStart.FlatStyle = FlatStyle.Flat;
            btnTdmStart.Size = new Size(110, 32);
            btnTdmStart.Location = new Point(8, 52);
            btnTdmStart.Click += btnTdmStart_Click;
            grpTdmStandalone.Controls.Add(btnTdmStart);

            btnTdmStop = new Button();
            btnTdmStop.Text = "STOP TDM";
            btnTdmStop.Font = new Font("Segoe UI", 9F, FontStyle.Bold);
            btnTdmStop.BackColor = Color.Red;
            btnTdmStop.ForeColor = Color.White;
            btnTdmStop.FlatStyle = FlatStyle.Flat;
            btnTdmStop.Size = new Size(100, 32);
            btnTdmStop.Location = new Point(123, 52);
            btnTdmStop.Click += btnTdmStop_Click;
            grpTdmStandalone.Controls.Add(btnTdmStop);

            lblTdmStatus = new Label();
            lblTdmStatus.Font = new Font("Consolas", 8F, FontStyle.Bold);
            lblTdmStatus.ForeColor = Color.DarkGreen;
            lblTdmStatus.Location = new Point(230, 52);
            lblTdmStatus.Size = new Size(270, 32);
            lblTdmStatus.BorderStyle = BorderStyle.FixedSingle;
            lblTdmStatus.BackColor = Color.White;
            lblTdmStatus.TextAlign = ContentAlignment.MiddleLeft;
            lblTdmStatus.Text = " TDM: Ready";
            grpTdmStandalone.Controls.Add(lblTdmStatus);

            // Row 3: Info
            Label lblInfo = new Label();
            lblInfo.Text = "Upload \u2192 Save EEPROM \u2192 device runs standalone after power cycle";
            lblInfo.Font = new Font("Segoe UI", 7F, FontStyle.Italic);
            lblInfo.ForeColor = Color.Gray;
            lblInfo.Location = new Point(8, 88);
            lblInfo.Size = new Size(w - 20, 14);
            grpTdmStandalone.Controls.Add(lblInfo);

            UpdateTdmHitsDisplay();
        }

        /// <summary>
        /// Update the TDM status label with calculated hits/second.
        /// Formula: With N active bands at D ms dwell, each band gets
        /// 1000 / (D * N) hits per second.  Cycle time = D * N ms.
        /// </summary>
        private void UpdateTdmHitsDisplay()
        {
            if (lblTdmStatus == null) return;

            int dwellMs = (numTdmDwell != null) ? (int)numTdmDwell.Value : 5;

            // Count active bands in the grid
            int activeBands = 0;
            if (dgvBands != null)
            {
                foreach (DataGridViewRow row in dgvBands.Rows)
                {
                    ...
                    if (active) activeBands++;
                }
            }

            if (activeBands == 0)
            {
                lblTdmStatus.Text = " No active bands";
                lblTdmStatus.ForeColor = Color.Gray;
                return;
            }

            double cycleMs = dwellMs * activeBands;
            double hitsPerSec = 1000.0 / cycleMs;

            lblTdmStatus.Text = string.Format(
                " {0} bands | {1}ms dwell | {2:F0} hits/s/band",
                activeBands, dwellMs, hitsPerSec);
            lblTdmStatus.ForeColor = Color.DarkGreen;
        }

        // ════════════════════════════════════════════════════════
        //  UPLOAD TO BOARD (background thread - prevents hang)
        // ════════════════════════════════════════════════════════
        private void btnTdmUpload_Click(object sender, EventArgs e)
        {
            ...
            // Collect band data from grid on UI thread
            List<byte[]> bandPackets = new List<byte[]>();
            int dwellMs = (numTdmDwell != null) ? (int)numTdmDwell.Value : 5;
            ...
            foreach (DataGridViewRow row in dgvBands.Rows)
            {
                ...
                double freqMhz = Convert.ToDouble(row.Cells["colStartMhz"].Value);
                double bwMhz = 20.0;
                int tones = 8;
                bool active = true;
                ...
                if (dgvBands.Columns.Contains("colTones") && row.Cells["colTones"].Value != null)
                    try { tones = Convert.ToInt32(row.Cells["colTones"].Value); } catch { }
                if (dgvBands.Columns.Contains("colActive") && row.Cells["colActive"].Value != null)
                    try { active = Convert.ToBoolean(row.Cells["colActive"].Value); } catch { }

                double loFreqHz = freqMhz * 1e6;
                float ddsBwHz = (float)(bwMhz * 1e6);
                uint ddsPoints = (uint)Math.Max(tones * 16, 32);
                float ddsStep = ddsBwHz / ddsPoints;
                float ddsCtrlFreq = ddsBwHz / 10.0f;

                // Build SET_TDM_BAND packet (26 bytes)
                byte[] data = new byte[26];
                data[0] = (byte)bandCount;
                Array.Copy(BitConverter.GetBytes(loFreqHz), 0, data, 1, 8);
                Array.Copy(BitConverter.GetBytes(ddsBwHz), 0, data, 9, 4);
                Array.Copy(BitConverter.GetBytes(ddsStep), 0, data, 13, 4);
                Array.Copy(BitConverter.GetBytes(ddsPoints), 0, data, 17, 4);
                Array.Copy(BitConverter.GetBytes(ddsCtrlFreq), 0, data, 21, 4);
                data[25] = (byte)(active ? 1 : 0);
                bandPackets.Add(data);

                bandCount++;
                Debug.WriteLine(string.Format("TDM band {0}: {1} MHz, BW={2}, Tones={3}",
                    bandCount, freqMhz, bwMhz, tones));
            }

            if (bandPackets.Count == 0)
            {
                MessageBox.Show("No valid bands found!", "Upload Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            // Disable buttons during upload
            if (btnTdmUpload != null) btnTdmUpload.Enabled = false;
            if (lblTdmStatus != null)
            {
                lblTdmStatus.Text = " Uploading " + bandPackets.Count + " bands...";
                lblTdmStatus.ForeColor = Color.DarkOrange;
            }

            // Build params packet
            byte[] paramData = new byte[7];
            paramData[0] = (byte)(pulseMode ? 1 : 0);
            paramData[1] = (byte)(dwellMs & 0xFF);
            paramData[2] = (byte)((dwellMs >> 8) & 0xFF);
            paramData[3] = (byte)(pulseOn & 0xFF);
            paramData[4] = (byte)((pulseOn >> 8) & 0xFF);
            paramData[5] = (byte)(pulseOff & 0xFF);
            paramData[6] = (byte)((pulseOff >> 8) & 0xFF);

            // Run upload on background thread to prevent UI freeze and device hang
            int totalBands = bandPackets.Count;
            Thread uploadThread = new Thread(() =>
            {
                try
                {
                    // Send each band one at a time with proper waits
                    for (int i = 0; i < bandPackets.Count; i++)
                    {
                        SendTdmCommand(CMD_SET_TDM_BAND, bandPackets[i], 26);
                        WaitForTdmQueueDrain(2000);   // wait for USB thread to send it
                        Thread.Sleep(100);              // firmware processing time

                        Debug.WriteLine("  Sent band " + (i + 1) + "/" + bandPackets.Count);
                    }

                    // Send TDM parameters
                    Thread.Sleep(100);
                    SendTdmCommand(CMD_SET_TDM_PARAMS, paramData, 7);
                    WaitForTdmQueueDrain(2000);
                    Thread.Sleep(100);

                    Debug.WriteLine("TDM Upload complete: " + totalBands + " bands, dwell=" + dwellMs + "ms");

                    // Update UI from UI thread
                    this.BeginInvoke((MethodInvoker)delegate
                    {
                        if (btnTdmUpload != null) btnTdmUpload.Enabled = true;
                        UpdateTdmHitsDisplay();
                        MessageBox.Show(string.Format(
                            "Uploaded {0} bands to STM32.\nDwell: {1}ms\n\n" +
                            "Click 'SAVE TO EEPROM' to make permanent,\n" +
                            "or 'START TDM' to test first.",
                            totalBands, dwellMs),
                ...
                catch (Exception ex)
                {
                    this.BeginInvoke((MethodInvoker)delegate
                    {
                        ...
                        if (lblTdmStatus != null)
                        {
                            lblTdmStatus.Text = " Upload FAILED";
                            lblTdmStatus.ForeColor = Color.Red;
                        }
            ...
            uploadThread.Name = "TdmUploadThread";
            uploadThread.Start();
        }

        // ════════════════════════════════════════════════════════
        //  SAVE TO EEPROM (band table + attenuator settings)
        // ════════════════════════════════════════════════════════
        private void btnTdmSaveEeprom_Click(object sender, EventArgs e)
        {
            if (!guiActive || usbConnection == null || !usbConnection.IsOpen)
            {
            ...
            var result = MessageBox.Show(
                "Save TDM band table + attenuator settings to EEPROM?\n\n" +
                "The board will auto-load this config on power-up\n" +
                "and start jamming standalone (no PC needed).",
                "Save TDM to EEPROM",
            ...
            try
            {
                // 1) Save TDM band table (firmware address 448+)
                SendTdmCommand(CMD_TDM_SAVE_EEPROM, null, 0);

                // 2) Save attenuator DAC values (DEVICE_CONFIG at EEPROM address 32)
                //    Same format as existing tlspbtConfigSave_Click
                ushort len = 0;
                byte[] write_data = new byte[64];
                ...
                write_data[len++] = tmpBuff[0]; write_data[len++] = tmpBuff[1];
                write_data[len++] = tmpBuff[2]; write_data[len++] = tmpBuff[3];
                ushort crc = nsAlexKir.MathLibrary.CRC.CalculateCRC16(write_data, len);
                write_data[len++] = (byte)((crc >> 0) & 0xFF);
                write_data[len++] = (byte)((crc >> 8) & 0xFF);
                ...
                if (lblTdmStatus != null)
                {
                    lblTdmStatus.Text = " EEPROM SAVED (bands + attenuators)";
                    lblTdmStatus.ForeColor = Color.DarkGreen;
                }
                Debug.WriteLine("TDM: Saved band table + attenuator settings to EEPROM");
            }
            catch (Exception ex)
            {
        ...
        //  LOAD FROM EEPROM
        // ════════════════════════════════════════════════════════
        private void btnTdmLoadEeprom_Click(object sender, EventArgs e)
        {
            if (!guiActive || usbConnection == null || !usbConnection.IsOpen)
            {
                MessageBox.Show("Device not connected!", "Not Connected",
            ...
            if (lblTdmStatus != null)
            {
                lblTdmStatus.Text = " EEPROM config loaded into RAM";
                lblTdmStatus.ForeColor = Color.DarkBlue;
            }
            Debug.WriteLine("TDM: Load from EEPROM sent");
        }

        // ════════════════════════════════════════════════════════
        //  START STANDALONE TDM
        // ════════════════════════════════════════════════════════
        private void btnTdmStart_Click(object sender, EventArgs e)
        {
            ...
            if (lblTdmStatus != null)
            {
                lblTdmStatus.Text = " TDM RUNNING (standalone)";
                lblTdmStatus.ForeColor = Color.Red;
            }
            Debug.WriteLine("TDM: Start sent");
        }
        ...
        private void btnTdmStop_Click(object sender, EventArgs e)
        {
            ...
            if (lblTdmStatus != null)
            {
                lblTdmStatus.Text = " TDM STOPPED";
                lblTdmStatus.ForeColor = Color.DarkGreen;
            }
            Debug.WriteLine("TDM: Stop sent");
        }
        ...
        private void ConfigureDDSForJamming(BandConfiguration firstBand)
        {
            try
            {
                ...
                // Wait for ALL DDS init commands to reach hardware
                WaitForCommandQueue(3000);
                Thread.Sleep(100);  // Extra settle time for AD9106

                // Ensure DDS is actively outputting
                AddSequences(new SequenceSetDDSCtrlFrequency(ltTxCommand, null, device.dds.FreqCtrl));
