...
{
	/* Don't override attenuator when TDM is active */
...
int dms_get_switch_state 				( void )
{
	...
	{
		/* Don't override PLL when TDM is actively hopping */
		if (TdmCfg.state != TDM_STATE_IDLE)
			return state;
	...
	switch ( pRxCmd->Type )
	{
		...
		case SET_SWEEP_PLL:			//{ Установка параметров свипа.
			if ( pRxCmd->Len>=18 )
			{
				...
				/* Only touch PLL hardware if TDM is NOT running.
				 * TDM controls PLL via tdm_hop_to_band().
				 * Without this guard, C# periodic polling overrides
				 * TDM's PLL hop with the Base frequency (2.4 GHz). */
				if (TdmCfg.state == TDM_STATE_IDLE)
				{
		...
		case SET_SWEEP_DDS:
			...
			{
				/* Only touch DDS hardware if TDM is NOT running.
				 * TDM controls DDS via tdm_configure_dds_ramp(). */
				if (TdmCfg.state == TDM_STATE_IDLE)
				{
		...
		case SET_PLL_REG:			//{	Запись регистра ФАПЧ.
			if ( pRxCmd->Len==4 ) 
			{
				/* Only touch PLL registers if TDM is NOT running */
				if (TdmCfg.state == TDM_STATE_IDLE)
				{
		...
		default:
			/* K9: Try TDM command handler first (commands 40-48) */
			if ( tdm_process_command(pRxCmd, pTxCmd) == 0 )
...
int dms_init							( void )
{
	...
	/* K9: Initialize TDM module (loads EEPROM, auto-starts if configured) */
	tdm_init();
...
static void dms_dds_control				( void )
{
	/* Don't run software DDS sweep when TDM is active */
	if (TdmCfg.state != TDM_STATE_IDLE)
	{
		dms_thread1_next_func();
		return;
...
static void dms_sweep_lo 				( void )
{
	/* Don't run standard LO sweep when TDM is actively hopping */
	if (TdmCfg.state != TDM_STATE_IDLE)
	{
		dms_thread2_next_func();
		return;
...
{
	/* ── TDM FAST PATH ──
	 * When TDM is actively jamming, bypass ALL housekeeping threads
	 * for maximum hop speed. No USB, ADC, LED, switch, or sweep polling.
	 * Only tdm_process() runs — pure hop loop.
	 *
	 * Housekeeping runs once every ~65536 hops as a heartbeat.
	 * At 3 bands / 150µs per hop: 65536 × 150µs × 3 ≈ 29 seconds.
	 * At 1 band / 100µs: 65536 × 100µs ≈ 6.5 seconds.
	 *
	 * USB STOP command not needed — device stops via power cycle or
	 * SAVE TO DEVICE re-sends config (CMD_TDM_LOAD_EEPROM stops first). */
	if (TdmCfg.state != TDM_STATE_IDLE)
	{
		tdm_process();

		/* ── Always poll USB so PC can send commands (STOP, reconfigure) ──
		 * Inline USB RX/TX check (~5-10µs) — negligible vs hop timing.
		 * Without this, device is unreachable during TDM.
		 * NOTE: Cannot call dms_check_recv_data()/dms_send_data() directly
		 * as they advance the thread1 state machine index. */
		if (usb_rx_counter >= SYSCOM_SIZE)
		{
			dms_decode_receive_command(usb_rx_buff);
			usb_rx_counter -= SYSCOM_SIZE;
		...
		/* Periodic housekeeping: ~every 65536 hops */
		if ((TdmCfg.hop_count & 0xFFFF) == 0 && TdmCfg.hop_count > 0)
		{
			/* Toggle LED as visual "I'm jamming" heartbeat */
			HAL_GPIO_TogglePin ( LED2_GPIO_Port, LED2_Pin );

			/* Read detector voltage for thermal monitoring */
			DeviceCfg.Detector.Voltage = detector_measure ( DeviceCfg.Detector.Ctrl );
		}
		return;
	}

	//	Standard operation — full housekeeping
	//	Поток №1
	...
	/* K9: TDM process - runs every iteration for accurate hop timing */
	tdm_process();
}
